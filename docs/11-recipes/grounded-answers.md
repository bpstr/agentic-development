...
For a small document set, keyword search or a database query can be sufficient. Add embeddings, hybrid search, or reranking when evaluation cases show the need. The [RAG chapter](../06-knowledge-and-memory/rag.md) explains those mechanisms.
...
Only consider [GraphRAG or Cognee](../06-knowledge-and-memory/graphs-and-cognee.md) when relationship reasoning, corpus summaries, or reusable memory make their additional indexing and operational work worthwhile. Keep source documents as the reference for extracted claims.
